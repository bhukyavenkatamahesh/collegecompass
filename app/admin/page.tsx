...
export default function AdminPage() {
  ...
  async function loadPayments(pg = paymentsPage) {
    setPaymentsLoading(true)
    const res = await fetch(`/api/admin/payments?page=${pg}&limit=20`)
    const data = await res.json()
    if (res.ok) {
      setPayments(data.payments ?? [])
      setPaymentStats(data.stats ?? paymentStats)
      setPaymentsPages(data.pages ?? 1)
    }
  ...
  useEffect(() => {
    if (loggedIn && tab === 'stats') loadPayments(paymentsPage)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loggedIn, tab, paymentsPage])
            ...
            {error && (
              <div
                style={{
                  ...
                  border: '1px solid rgba(224,72,60,0.2)',
                  borderRadius: 8,
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div className="container" style={{ paddingTop: '2.5rem', paddingBottom: '5rem' }}>
        ...
        >
          {[
            [money(paymentStats.revenueTodayPaise), 'Revenue Today'],
            [
              paymentStats.predictions > 0 ? paymentStats.predictions.toLocaleString() : '—',
              'Total Predictions',
            ],
            [
              paymentStats.paidCount > 0 ? paymentStats.paidCount.toLocaleString() : '—',
              'Paid Payments',
            ],
            [totalCount > 0 ? totalCount.toLocaleString() : '—', 'Cutoff Entries'],
          ].map(([v, l]) => (
          ...
          <>
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                ...
                <button onClick={() => loadCutoffs(page, search)} className="btn btn-ghost btn-sm">
                  ↻
                </button>
            ...
            {showAdd && (
              <div
                className="glass"
                style={{ padding: '1.8rem', borderRadius: 'var(--r)', marginBottom: '1.5rem' }}
              >
                <h3 style={{ fontSize: '1.05rem', marginBottom: '1.2rem' }}>
                  Add New Cutoff Entry
                </h3>
                <div className="admin-cutoff-grid">
                  {cutoffFields.map(({ key, label, options }) => (
                    <div key={key}>
                      ...
                      {options ? (
                        <select
                          value={String(newCutoff[key])}
                          onChange={e => setNewCutoff(p => ({ ...p, [key]: e.target.value }))}
                          className="input-field"
                ...
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  <button onClick={addCutoff} className="btn btn-primary btn-sm">
                    Save Entry
                  </button>
                    ...
                    {cutoffsLoading ? (
                      ...
                      cutoffs.map(c => (
                        ...
                        >
                          ...
                          <td
                            style={{ padding: '0.75rem 1rem', fontVariantNumeric: 'tabular-nums' }}
                          >
        ...
        {tab === 'stats' && (
          <>
            ...
            >
              <div>
                <h2 style={{ fontSize: '1.3rem' }}>Payments</h2>
                <p
                  style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '0.25rem' }}
                >
                  Total revenue {money(paymentStats.revenuePaise)} across{' '}
                  {paymentStats.paidCount.toLocaleString()} paid payments.
                </p>
              </div>
              <button onClick={() => loadPayments(paymentsPage)} className="btn btn-ghost btn-sm">
                Refresh
              </button>
            </div>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                  <thead>
                    ...
                    >
                      {['Customer', 'Exam', 'Amount', 'Status', 'Razorpay IDs', 'Created'].map(
                        h => (
                          <th
                            key={h}
                            style={{
                  ...
                  <tbody>
                    {paymentsLoading ? (
                      ...
                      <tr>
                        ...
                        >
                          No payment records yet. Complete a test checkout to see it here.
                        </td>
                      </tr>
                    ) : (
                      payments.map(payment => (
                        ...
                        >
                          <td style={{ padding: '0.75rem 1rem' }}>
                            <div
                              style={{ fontWeight: 700, fontFamily: 'Satoshi,Inter,sans-serif' }}
                            >
                              {payment.user.name}
                            </div>
                          ...
                          <td style={{ padding: '0.75rem 1rem' }}>
                            <span className="tag">{payment.examType}</span>
                          </td>
                          <td
                            style={{
                              padding: '0.75rem 1rem',
                          ...
                          >
                            <div>{payment.razorpayOrderId}</div>
                            <div>{payment.razorpayPaymentId ?? '—'}</div>
                          </td>
                          <td
                            style={{
                              padding: '0.75rem 1rem',
            ...
            {paymentsPages > 1 && (
              ...
              >
                ...
                >
                  Page {paymentsPage} of {paymentsPages} ·{' '}
                  {paymentStats.totalCount.toLocaleString()} payment records
                </span>
                <div style={{ display: 'flex', gap: '0.4rem' }}>
                  <button
