...
const BRANCH_MAP: Record<string,string> = { ALL:'All branches', CS:'Computer Science', EC:'Electronics & Comm.', EE:'Electrical Engineering', ME:'Mechanical Engineering', CE:'Civil Engineering', CH:'Chemical Engineering', BT:'Biotechnology', MA:'Mathematics', DA:'Data Science & AI', MATH:'Mathematics / Computing', PHYSICS:'Physics' }
...
function ResultsContent() {
  ...
  useEffect(() => {
    ...
    fetch('/api/predict', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) })
      .then(r=>r.json())
      .then(d=>{ setColleges(d.locked ? [] : d.results || []); setLoading(false) })
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)', color:'var(--text)', paddingBottom:'4rem' }}>
      ...
      <div className="nav-wrapper">
        <nav className="nav">
          ...
          <div className="hide-sm" style={{ fontSize:'0.85rem', color:'var(--text-muted)', fontFamily:'Satoshi,Inter,sans-serif', fontWeight:600 }}>Your {examLabel} Results</div>
              ...
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'0.875rem', textAlign:'left' }}>
                <thead>
                  <tr style={{ background:'var(--bg-muted)', borderBottom:'1px solid var(--border)' }}>
                    <th className="col-hide-mobile" style={{ padding:'0.9rem 0.75rem', fontFamily:'Satoshi,Inter,sans-serif', fontWeight:700, fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.06em', color:'var(--text-muted)', whiteSpace:'nowrap' }}>#</th>
                    <th style={{ padding:'0.9rem 0.75rem', fontFamily:'Satoshi,Inter,sans-serif', fontWeight:700, fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.06em', color:'var(--text-muted)', whiteSpace:'nowrap' }}>Institute</th>
                    <th style={{ padding:'0.9rem 0.75rem', fontFamily:'Satoshi,Inter,sans-serif', fontWeight:700, fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.06em', color:'var(--text-muted)', whiteSpace:'nowrap' }}>Program</th>
                    <th className="col-hide-mobile" style={{ padding:'0.9rem 0.75rem', fontFamily:'Satoshi,Inter,sans-serif', fontWeight:700, fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.06em', color:'var(--text-muted)', whiteSpace:'nowrap' }}>Type</th>
                    <th className="col-hide-mobile" style={{ padding:'0.9rem 0.75rem', fontFamily:'Satoshi,Inter,sans-serif', fontWeight:700, fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.06em', color:'var(--text-muted)', whiteSpace:'nowrap' }}>State</th>
                    <th className="col-hide-mobile" style={{ padding:'0.9rem 0.75rem', fontFamily:'Satoshi,Inter,sans-serif', fontWeight:700, fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.06em', color:'var(--text-muted)', whiteSpace:'nowrap' }}>{isGate?'Min Score':'Open Rank'}</th>
                    <th style={{ padding:'0.9rem 0.75rem', fontFamily:'Satoshi,Inter,sans-serif', fontWeight:700, fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.06em', color:'var(--text-muted)', whiteSpace:'nowrap' }}>{isGate?'Max Score':'Close Rank'}</th>
                    <th style={{ padding:'0.9rem 0.75rem', fontFamily:'Satoshi,Inter,sans-serif', fontWeight:700, fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.06em', color:'var(--text-muted)', whiteSpace:'nowrap' }}>Chance</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((c,i)=>(
                    <tr key={i} style={{ borderBottom:'1px solid var(--border)' }}
                      ...
                      <td className="col-hide-mobile" style={{ padding:'0.85rem 0.75rem', color:'var(--text-faint)', fontVariantNumeric:'tabular-nums' }}>{i+1}</td>
                      <td style={{ padding:'0.85rem 0.75rem', fontWeight:600, fontFamily:'Satoshi,Inter,sans-serif', minWidth:140 }}>
                        {c.institute}
                        ...
                        <div style={{ display:'none' }} className="show-mobile-inline">
                          <span className="tag" style={{marginTop:'0.25rem',display:'inline-block'}}>{c.instituteType}</span>
                        </div>
                      </td>
                      <td style={{ padding:'0.85rem 0.75rem', color:'var(--text-muted)', minWidth:130 }}>
                        {c.program}
                      ...
                      <td className="col-hide-mobile" style={{ padding:'0.85rem 0.75rem', color:'var(--text-muted)', whiteSpace:'nowrap' }}>{c.state}</td>
                      <td className="col-hide-mobile" style={{ padding:'0.85rem 0.75rem', fontVariantNumeric:'tabular-nums' }}>{isGate ? Math.min(c.openScore,c.closeScore) : c.openRank?.toLocaleString()}</td>
                      <td style={{ padding:'0.85rem 0.75rem', fontVariantNumeric:'tabular-nums' }}>{isGate ? Math.max(c.openScore,c.closeScore) : c.closeRank?.toLocaleString()}</td>
                      <td style={{ padding:'0.85rem 0.75rem' }}>
                        <span style={{ background:chanceBg(c.chance), color:chanceColor(c.chance), borderRadius:8, padding:'0.28rem 0.6rem', fontSize:'0.78rem', fontWeight:700, fontFamily:'Satoshi,Inter,sans-serif', whiteSpace:'nowrap' }}>
                          {c.chancePercent}% {c.chance}
                        </span>
                      </td>
                    </tr>
                  ...
                  {filtered.length === 0 && (
                    <tr><td colSpan={8} style={{ padding:'3rem 1rem', textAlign:'center', color:'var(--text-muted)' }}>No colleges match the selected filters.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
