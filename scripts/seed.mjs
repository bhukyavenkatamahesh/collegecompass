...
async function main() {
  ...
  console.log(
    `Seeded ${records.length} cutoff entries (${GATE_CUTOFFS.length * CATEGORIES.length} GATE + ${JEE_CUTOFFS.length * CATEGORIES.length} JEE across ${CATEGORIES.length} categories).`
  )
}
